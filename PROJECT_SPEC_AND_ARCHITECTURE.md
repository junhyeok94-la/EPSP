...
* **주문 식별자 (`order_id`) - 트랜잭션 수집 및 정산용**:
  ...
  * 이유: 개별 주문서의 상태 변경 트래킹(`CREATED` -> `PAID` -> `SHIPPED`)은 주문 번호(`order_id`) 단위의 정렬이 중요합니다. 한 사용자가 독립적인 주문 여러 건을 수행할 때, 각 주문 트랜잭션은 상호 독립적이므로 테이블의 PK인 `order_id`를 파티션 키로 사용하여 처리 병렬성을 확장합니다.
* **안티패턴 경고 (`product_id`를 카프카 파티션 키로 지정 금지)**:
  * 유저의 실시간 클릭 스트림에서 `product_id`를 카프카 파티션 키로 삼을 경우, 한 사용자가 조회하는 다양한 상품 이벤트가 서로 다른 카프카 파티션으로 분산됩니다. 이 경우 Flink/Spark Streaming이 특정 유저의 최근 5분간 상품 클릭 목록을 윈도우 조인하려 할 때 데이터 순서가 완전히 뒤엉키며, 네트워크 셔플(Shuffle)이 기하급수적으로 늘어나 성능 저하를 초래합니다.

##### 2. ClickHouse(DW) 테이블 파티셔닝 (`PARTITION BY`) 설계 원칙
ClickHouse의 파티션은 카프카와 완전히 다른 목적(물리 디스크 저장 격리 및 스캔 최적화)을 갖습니다. 잘못 지정할 경우 시스템 프리징을 초래합니다.

* **안티패턴 경고 (고카디널리티 컬럼 파티셔닝 금지)**:
  * ClickHouse 테이블 생성 시 `user_id`나 `product_id` 같이 고유값이 수백만 개가 넘는 컬럼을 `PARTITION BY`로 정의하면 안 됩니다.
  * ClickHouse는 파티션별로 디스크 상에 별도 물리 폴더(Part)를 생성합니다. 고카디널리티 컬럼 지정 시 백그라운드 머지 스레드가 감당할 수 없을 정도로 많은 폴더가 생성되어 **"Too many parts"** 예외와 함께 DB가 다운됩니다.
* **표준 가이드라인**:
  * **물리 파티셔닝 키 (`PARTITION BY`)**: 카디널리티가 현저히 낮은 **시간 범위**(`toYYYYMM(created_at)`)나 대분류 도메인 코드로 설정하여, 스캔 최적화(Data Pruning) 및 대용량 데이터 일괄 보관 주기 Drop 처리에 대응합니다.
  * **희소 인덱스 정렬 키 (`ORDER BY`)**: 쿼리의 고속 필터링 및 조인을 수행하기 위해, 검색 기준이 되는 **`(product_id, created_at)`** 또는 **`(user_id, created_at)`** 조합을 정렬 키로 지정하여 인덱스를 튜닝합니다.

##### 3. 이종 도메인 간 스트림 조인 및 정렬 표준 (Re-keying & Stateful Join)
카프카에서 각기 다른 파티션 키를 가져 순서 보장이 분리된 두 스트림(예: `order_id`가 키인 주문 스트림과 `delivery_id`가 키인 배송 스트림)을 결합해야 하는 비즈니스 도메인의 경우 Flink/Spark 수준에서 다음과 같이 조율해야 합니다.

* **Re-keying (재분배)**:
  * 스트림 프로세서 내부에서 공통 조인 키(예: `order_id`)를 기준으로 **`keyBy(order_id)`** 연산을 선언하여 데이터 스트림을 실시간으로 메모리 상에서 재정렬(Repartitioning)합니다.
* **상태 기반 윈도우 조인 (Stateful Window Join)**:
  * 서로 다른 시점에 들어오는 이벤트들을 처리하기 위해 Flink의 내부 메모리 상태(State)에 선도착 이벤트를 버퍼링하고, 시간 윈도우(예: 30분) 범위 내에서 결합을 완수한 후 결과 레이어에 적재합니다.
* **최종 화면 일관성 (Eventual Consistency) 제어**:
  * 백엔드 API 및 프론트엔드 UI는 데이터 파이프라인의 비비동기 지연을 전제하고 설계해야 합니다. (예: 결제 직후 데이터 정합성이 맞춰지기 전까지 '결제 확인 중' 상태 스피너 연출, 또는 WebSocket 기반 푸시 구독 구조 채택).
...
실시간 스트림 처리 엔진 (Apache Flink 1.17.1): Kafka 토픽에서 실시간으로 이벤트를 스트리밍하고 집계 연산을 분산 처리하는 전용 분석 컨테이너입니다. Flink의 연산 태스크 현황을 시각화 모니터링하기 위해 웹 대시보드 포트인 8081을 활성화합니다.

카프카 시각화 모니터링 (Kafka UI): Kafka 브로커 상태, 토픽 리스트, 파티션 분배 및 Debezium Connect의 싱크/소스 커넥터 구동 현황을 한눈에 추적하는 웹 UI 도구입니다. 노트북에 기동 중인 Airflow 웹서버(8080)와의 포트 충돌을 피하기 위해 외부 호스트 8089 포트로 연결을 격리 제공합니다.
