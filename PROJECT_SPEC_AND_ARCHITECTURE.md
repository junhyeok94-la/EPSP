...
2. **논리 관계의 문서화**: ClickHouse와 같이 물리 외래키 제약이 없는 환경의 경우, dbt `schema.yml` 내 `tests: - relationships`를 통해 논리 관계를 정형화하고, 마스터 카탈로그 문서인 [DATA_CATALOG.md](file:///d:/01.DEV/EPSP/notebook/DATA_CATALOG.md) 파일에 릴레이션 매핑 테이블을 지속 유지합니다.
